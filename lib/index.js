...
function updated(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    const jsonFilePath = path.isAbsolute(config.updatedFile) ? config.updatedFile : path.join(metalsmith.source(), config.updatedFile)
    ...
    updates = {};
    try {
      const jsonUpdateFile = fs.readFileSync(jsonFilePath);
      updates = JSON.parse(jsonUpdateFile);
    ...
    function () {
      ...
      jsonfile.writeFileSync(jsonFilePath, updates);
      removeFiles(files, config);
      done();
    });
  }
